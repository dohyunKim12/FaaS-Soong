...
def draw_bbox(image, bboxes,client_socket , classes=read_class_names(cfg.YOLO.CLASSES), show_label=True):
    ...
    for i in range(num_boxes[0]):
        ...
        cropped_img = image[int(coor[0]): int(coor[2]) + 50, int(coor[1]):int(coor[3]) + 50]
        ...
        if show_label:
            ...
            if classes[class_ind] == 'person':
                img_name = "c:/tensorflow-yolov4-tflite-master/crop_image/person.jpg" #이미지 지정 폴더 설정
                cv2.imwrite(img_name,cropped_img)
                send_img(client_socket,img_name)
            cv2.putText(image, bbox_mess, (c1[0], np.float32(c1[1] - 2)), cv2.FONT_HERSHEY_SIMPLEX,
                        fontScale, (0, 0, 0), bbox_thick // 2, lineType=cv2.LINE_AA)
    return image

def send_img(clieanant_socket,img_name):
    FILE_SIZE = os.path.getsize(img_name)
    f = open(img_name, 'rb')
    I = f.read(1024)
    fs = str(FILE_SIZE).encode()
    clieanant_socket.send(fs)  # 사이즈 보내기
    time.sleep(0.01)
    while I:
        clieanant_socket.send(I) #
        I = f.read(1024)
    f.close()

    print('(check) ' + img_name + ' transfer complete')
    server_msg = clieanant_socket.recv(1024)
