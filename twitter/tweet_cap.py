...
def upload_tweet(api):
    ...
    for i in range(len(data["fire"])):
        ...
        media2 = api.media_upload("cap.jps")
        status = api.update_status(status=msg, media_ids=[media.media_id,media2_id])
...
def make_msg(kind):
    ...
    msg += "사고 위치 지도에서 보기 : http://amp.paasta.koren.kr/map.php?latitude=" + lati + "&longitude=" + longi
