...
video = './test.mp4'
weight = './yolov4-faasoong_final.weights'
cfg = './cfg/yolov4-faasoong.cfg'
data = './data/obj.data'
...
def inference(darknet_image_queue, detections_queue, fps_queue):
    ...
    while cap.isOpened():
        ...
        for i in detections:
            ...
            else: pass #accident, fire, people, gun, knife = 0,0,0,0,0
        if accident == 5 :
            trigger('accident')
            #accident = 0
        elif fire == 5 :
...
def trigger(detecting_object):
    ...
    occur_date = datetime.datetime.now().strftime('%Y-%m-%d')
    occur_time = datetime.datetime.now().strftime('%H:%M:%S')
    db_url = "http://amp.paasta.koren.kr/"+detecting_object+"_insert.php"
    params = { "date" : occur_date, "time" : occur_time, "latitude" : latitude, "longitude" : longitude }
    ...
    channel.basic_publish(exchange='', routing_key=detecting_object, body='')
